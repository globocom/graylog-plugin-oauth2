...
const OAuth2Configuration = React.createClass({
  ...
  _bindChecked(ev, value) {
    this._setSetting(
      ev.target.name,
      typeof value === "undefined" ? ev.target.checked : value
    );
  },
  ...
  render() {
    const toggleButtonText = this.state.showSettings
      ? "Group Mapping"
      : "OAuth2 Settings";
    let content;
      ...
      <DocumentTitle title="Oauth2">
        <span>
          <PageHeader title="Oauth2" subpage>
            <span>
              This page is the only resource you need to set up the Graylog
              OAuth2 integration.
            </span>
            {null}
            <span>
              <Button bsStyle="success" onClick={this._toggleButton}>
                {toggleButtonText}
              </Button>
            </span>
          </PageHeader>
